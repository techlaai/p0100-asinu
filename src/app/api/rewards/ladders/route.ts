...
import { fetchActiveLadder } from "@/modules/rewards/service";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const ladder = await fetchActiveLadder();
    return jsonSuccess({ ladder }, { request: req, cacheControl: "no-store" });
  } catch (error) {
    console.error("[rewards/ladders] failed", error);
